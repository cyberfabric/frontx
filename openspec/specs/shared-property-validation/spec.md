...
- **WHEN** the host calls `updateSharedProperty(propertyId, value)` with a value that conforms to the property's derived schema (e.g., `"dark"` for theme)
...
- **AND** the property value SHALL be stored and propagated to all matching domains' subscribers

#### Scenario: Invalid property value is rejected

- **WHEN** the host calls `updateSharedProperty(propertyId, value)` with a value that violates the property's derived schema (e.g., `"neon"` for theme when the schema defines `enum: ["default", "light", "dark", "dracula", "dracula-large"]`)
...
- **AND** `updateSharedProperty()` SHALL throw an error containing the validation failure details
...
- **WHEN** validating a shared property value via `updateSharedProperty`
...
#### Scenario: Validation MAY be performed once per property type

- **WHEN** `updateSharedProperty(propertyId, value)` is called and multiple registered domains declare `propertyId` in their `sharedProperties` array
- **THEN** the implementation MAY perform GTS validation once for the property type rather than repeating it per matching domain
- **AND** this is valid because the GTS schema is derived from the property type ID, which is identical regardless of which domain declares the property
- **AND** validation MUST complete successfully before propagation begins to any domain

#### Scenario: Unregistered property schema is a configuration error

- **WHEN** the host calls `updateSharedProperty(propertyId, value)` where `propertyId` refers to a derived schema that has NOT been registered in the GTS store (e.g., a custom `gts...color.v1~` property type whose schema was never loaded)
...
- **AND** `updateSharedProperty()` SHALL throw an error indicating the schema could not be found for the property type
...
- **WHEN** `updateSharedProperty()` is called multiple times for the same property type ID with different values
