...
| Tech-writer | `AGENTS.md`, `AGENTS.md`, `.Codex/agents/`, `.Codex/skills/`, content fixes in `architecture/` |
...
**Onboard, don't micromanage.** When spawning an agent, include task-specific orientation: what exists in the area they'll work on, the relevant architecture artifact, and the current state of that piece. Don't repeat repo-wide conventions — those are already in AGENTS.md and agent prompts.
