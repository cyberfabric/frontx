...
1) Check if existing UI components cover the need (see CHECK EXISTING COMPONENTS).
2) Gather requirements from user.
3) Implement.

## CHECK EXISTING COMPONENTS FIRST
- REQUIRED: Before creating a new component, check for an equivalent in the MFE (e.g. `components/ui/`).
- REQUIRED: Reuse or extend existing local components when available.
