...
**Constraints:** HAI3 uses `@reduxjs/toolkit@2.11.2` as a devDependency in `@hai3/state` and `@hai3/react`. The store is a single global instance created via `createStore()` in `@hai3/state` with dynamic slice registration. All MFEs share the host's `QueryClient` for cross-MFE cache deduplication (see `HAI3Provider.tsx` and `MfeProvider.tsx`). Redux state uses a `screensetId/domain` namespace convention for MFE isolation.
...
In HAI3, the store is created via `createStore()` in `@hai3/state` (L1). Adding RTK Query's reducer and middleware would require modifying the store creation logic. Since `@hai3/state` is an L1 package with zero `@hai3` dependencies, adding RTK Query integration here would keep it at L1 (RTK is already a peer dependency). However, RTK Query's `createApi` with React hooks comes from `@reduxjs/toolkit/query/react`, which has a React dependency -- this would need to live at L3.
...
**Confidence:** Substantiated -- based on RTK Query architecture and reading the HAI3 `@hai3/state` store implementation.
...
2. **Store modification scope.** If RTK Query were adopted, how much modification to `@hai3/state`'s `createStore()` would be needed to accommodate the API reducer and middleware? The current API does not expose middleware configuration.
