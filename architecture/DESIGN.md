...
| `cpt-frontx-fr-framework-query-cache-plugin` | `queryCache(config?)` plugin owns a QueryClient-backed `ServerStateRuntime`; exposes `app.serverState`; clears cache on mock toggle; handles `cache/invalidate`/`cache/set`/`cache/remove` events and keeps `sharedFetchCache` in sync |
...
| `cpt-frontx-fr-react-query-client-isolation` | `HAI3Provider` reads `app.serverState` from plugin; separately mounted MFEs receive the same host `ServerStateRuntime` through an opaque per-link `mountRuntimeToken` side channel (symbol-backed map primed before `mount_ext`, consumed once in the child React lifecycle) and expose only the restricted `QueryCache` interface |
...
- **HAI3Provider**: Root provider component that wraps the application with Redux store, i18n context, theme, framework context, and a `ServerStateProvider` bridge. `HAI3Provider` does not create a TanStack client; `queryCache()` is the sole owner of the QueryClient-backed `ServerStateRuntime`, and the provider reads `app.serverState` or accepts the same host-owned `serverState` prop for separately mounted MFE roots so every root reuses the plugin-owned cache. During `mount_ext`, the framework stores `{ serverState }` in a symbol-backed global map under an **opaque per-link `mountRuntimeToken`** (assigned on the actions-chain link), and the child React lifecycle consumes that token once from `MfeMountContext` before rendering its own provider tree (with a narrow fallback to `app.serverState` when extension/domain are set but no token was supplied). The TanStack adapter mounted inside `ServerStateProvider` owns the actual `QueryClientProvider`, while `sharedFetchCache` deduplicates overlapping descriptor fetches across roots before React observers attach.
- **Hooks**: `useSelector()`, `useDispatch()`, `useTranslation()`, `useSharedProperty()`, `useAction()` — typed wrappers over framework primitives
- **Query hooks**: `useApiQuery()` for single-page declarative reads, `useApiSuspenseQuery()` for Suspense-driven single-page reads, `useApiInfiniteQuery()` for descriptor-driven paginated reads, `useApiSuspenseInfiniteQuery()` for Suspense-driven paginated reads, `useApiMutation()` for writes with optimistic updates via `QueryCache`, and `useQueryCache()` as the sanctioned imperative cache API. `QueryCache` exposes `get`, `getState`, `set`, `cancel`, `invalidate`, `invalidateMany`, and `remove`. The raw `QueryClient` stays internal behind `ServerStateRuntime`, `useQueryClient` is NOT exported from `@cyberfabric/react`, and app/MFE code uses `QueryCache` rather than raw TanStack APIs.
