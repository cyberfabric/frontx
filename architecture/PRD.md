...
`@cyberfabric/framework` MUST provide a `queryCache(config?)` framework plugin that creates and owns a QueryClient-backed `ServerStateRuntime` from `@tanstack/query-core`. The plugin MUST expose `app.serverState` via the registries mechanism, accept configurable defaults (`staleTime`, `gcTime`, `refetchOnWindowFocus`), clear the cache on `MockEvents.Toggle`, handle `cache/invalidate`, `cache/set`, and `cache/remove` events from L2 Flux effects, keep the L1 `sharedFetchCache` in sync for cross-root reads, and clean up on `app.destroy()`. The plugin MUST be included in the `full()` preset.
...
`HAI3Provider` MUST read `app.serverState` from the `queryCache()` framework plugin instead of creating its own cache runtime. When used inside separately mounted MFE React roots, the host MUST hand off the same `ServerStateRuntime` instance via an opaque per-`mount_ext` link **`mountRuntimeToken`**, so overlapping queries (same descriptor key) are deduplicated and cached once across MFE boundaries. Before the wrapped `executeActionsChain` runs, the framework MUST prime `{ serverState }` in the symbol-backed mount-runtime store under each such token; `MfeMountContext` MUST carry `mountRuntimeToken` into `MfeEntryLifecycle.mount(container, bridge, mountContext)`, and the child React lifecycle MUST consume that entry (one-shot removal) and pass the resolved runtime into `HAI3Provider` (with a defined fallback to `app.serverState` when extension/domain are known but no token applies). The framework MUST clear any leftover map entries for primed tokens in a `finally` block after the chain runs. Each MFE retains its own `apiRegistry` and service instances — the descriptor's `fetch` uses the local service, but the shared runtime and L1 `sharedFetchCache` coordinate deduplication across roots. MFEs MUST NOT access the native cache client directly — cache operations are available only through the `QueryCache` interface (`get`, `getState`, `set`, `cancel`, `invalidate`, `invalidateMany`, `remove`) which accepts `EndpointDescriptor | QueryKey`. Cache keys are derived automatically from `[baseURL, 'GET', path]` for static endpoints, `[baseURL, 'GET', resolvedPath, params]` for parameterized endpoints, `[baseURL, method, path]` for writes, and `[baseURL, 'SSE', path]` for streams — MFEs sharing the same derived key share cache entries; MFEs wanting isolated cache use a different `baseURL`.

**Rationale**: MFEs render in separate React roots, so they cannot share the cache through React-context inheritance alone. The `queryCache()` plugin owns the `ServerStateRuntime` (with TanStack `QueryClient` under the hood) and makes it available to `HAI3Provider`. The restricted `QueryCache` interface prevents uncontrolled cross-MFE cache tampering. Cache key derivation from `baseURL` gives MFEs natural opt-in/opt-out control over shared caching.
**Actors**: `cpt-frontx-actor-host-app`, `cpt-frontx-actor-runtime`
