---
type: workflow
name: migrate-openspec-to-cypilot
version: 2.0
purpose: Migrate OpenSpec artifacts to Cypilot SDLC standard documents (PRD, DESIGN, ADR, FEATURE, DECOMPOSITION) with code-verified traceability
system: hai3
---
...
**Input**: OpenSpec artifacts at `openspec/` (24 main specs, 82+ archived changes) + source code at `packages/` and `src/`
**Output**: Cypilot artifacts at `architecture/` + `@cpt-*` code markers in source files, registered in `.cypilot/config/artifacts.toml` with `FULL` traceability
...
- [ ] Cypilot initialized (`.cypilot/` exists with `config/artifacts.toml`)
- [ ] OpenSpec directory exists at `openspec/` with main specs and archived changes
- [ ] Source code available at `packages/` and `src/`
- [ ] Agent has read this workflow in full before starting any phase
- [ ] Agent has access to Cypilot templates at `.cypilot/.gen/kits/sdlc/artifacts/`
...
|---|---|---|
| Main specs (`openspec/specs/*/spec.md`) | **PRD** § Functional Requirements | Each spec requirement → one `cpt-hai3-fr-*` ID |
| Main specs (non-functional aspects) | **PRD** § Non-Functional Requirements | Performance, reliability, security constraints → `cpt-hai3-nfr-*` IDs |
...
| Design architecture (layers, components) | **DESIGN** § Technical Architecture | Component descriptions → `cpt-hai3-component-*` IDs |
| Design principles + constraints | **DESIGN** § Principles & Constraints | Explicit or implicit → `cpt-hai3-principle-*`, `cpt-hai3-constraint-*` IDs |
| Task categories (`tasks.md`) | **DECOMPOSITION** entries | Each `## N. Category` → one feature entry with `cpt-hai3-feature-*` ID |
| Task details + spec scenarios | **FEATURE** specs (per feature) | Given/When/Then scenarios → CDSL flows with `cpt-hai3-flow-*` IDs |
...
ADR cross-cuts: each `cpt-hai3-adr-*` referenced in DESIGN § Architecture Drivers.
...
**Fail-safe**: If context approaches capacity mid-phase, write a checkpoint file at `architecture/workflows/.migrate-checkpoint.md` and resume from there.
...
1. **Locate**: Find the source file(s) that implement it (search `packages/` and `src/`)
...
|---|---|---|---|---|---|
| 2 | cpt-hai3-fr-routing-lazy | ADJUSTED | SHALL lazy-load all routes | Only lazy-loads MFE routes | FR updated to match code |
```
...
Read the `openspec/specs/` directory. For each capability:
...
Scan `packages/` and `src/` to understand the implementation landscape:

1. List all packages with their entry points and line counts
2. Map packages to OpenSpec capability areas
...
|---|---|---|---|---|
| @hai3/state | packages/state/src/index.ts | 1200 | sdk-core | Covered |
| @hai3/api | packages/api/src/index.ts | 800 | sdk-core | Covered |
| @hai3/studio | packages/studio/src/index.ts | 3000 | studio | Partial |
```

### Step 1.3: Catalog Change Artifacts

Scan `openspec/changes/archive/` for design decisions and task structures:
...
**Source**: `openspec/config.yaml` → `context` field + earliest proposals
**Target**: PRD §1 Overview (Purpose, Background, Goals, Glossary)

1. Read `openspec/config.yaml` `context` field → extract project description
...
2. Create `cpt-hai3-actor-{slug}` ID for each distinct actor
3. Write actor descriptions with responsibilities and interaction patterns

**Transformation rules**:
- Spec references to "the developer" → `cpt-hai3-actor-developer`
- Spec references to "the host application" → `cpt-hai3-actor-host-app`
- Spec references to "the build system" → `cpt-hai3-actor-build-system`
...
**Source**: Each `openspec/specs/*/spec.md` → `### Requirement:` headings
**Verification**: Actual source code in `packages/` and `src/`
**Target**: PRD §5 Functional Requirements with `cpt-hai3-fr-*` IDs

For each main spec capability:

1. Read the spec file
2. For each `### Requirement:` heading:
   a. Generate ID: `cpt-hai3-fr-{capability}-{requirement-slug}`
   b. **Locate the implementing code** — find the source file(s) in `packages/` that implement this requirement
...
**ID format**: `- [ ] \`p{N}\` - **ID**: \`cpt-hai3-fr-{capability}-{slug}\``
...
**Target**: PRD §6 Non-Functional Requirements with `cpt-hai3-nfr-*` IDs
...
3. Generate `cpt-hai3-nfr-{category}-{slug}` IDs
...
|---|---|---|
| §3 Operational Concept | `config.yaml` context, design files | Verify against `package.json` workspaces, build config |
...
| §10 Dependencies | `package.json`, design external deps | **Read actual `package.json`** files |
...
- [ ] All IDs follow `cpt-hai3-{kind}-{slug}` format
...
**Source**: `openspec/config.yaml` context, design files, PRD from Phase 2
**Target**: DESIGN §1 Architecture Overview

1. Synthesize architectural vision from config context (monorepo, layers, patterns)
2. Map PRD `fr` and `nfr` IDs → Architecture Drivers (coverage required)
3. Extract architecture layers from config: L1 SDK → L2 Framework → L3 React → UI Layer → Tooling
4. **Verify layers against actual package structure** — confirm `packages/` directories match the described layers

**Transformation rules**:
- Every `cpt-hai3-fr-*` ID from PRD MUST appear in §1.2.a Functional Drivers
- Every `cpt-hai3-nfr-*` ID from PRD MUST appear in §1.2.b NFR Allocation
...
**Verification**: Actual code patterns in `packages/`
**Target**: DESIGN §2 Principles & Constraints

1. Extract explicit principles from `config.yaml` architecture patterns:
   - Event-driven flow → `cpt-hai3-principle-event-driven`
   - Layer isolation → `cpt-hai3-principle-layer-isolation`
   - Zero cross-dependencies at L1 → `cpt-hai3-constraint-l1-no-cross-deps`
2. **Verify each principle/constraint against code**:
   - Does the code actually follow event-driven patterns? Check for event bus, dispatch calls
   - Does L1 actually have zero cross-deps? Check imports in `packages/state/`, `packages/api/`, etc.
...
**ID format**: `cpt-hai3-principle-{slug}` and `cpt-hai3-constraint-{slug}`

### Step 3.3: Extract Component Model (Code-Verified)

**Source**: Main specs (each capability ≈ component), package.json workspaces
**Verification**: Actual package structure, entry points, exports
**Target**: DESIGN §3.2 Component Model

For each logical component:

1. Generate `cpt-hai3-component-{slug}` ID
2. **Read the actual package** — examine `package.json`, `src/index.ts`, key source files
3. Write 4 required subsections based on what the code actually does:
   ...
   - **Related components** (from actual import graph between packages)

**Mapping heuristic**:
- Each `@hai3/*` package → one component (e.g., `cpt-hai3-component-state`, `cpt-hai3-component-api`)
- Cross-cutting capabilities → grouped component (e.g., `cpt-hai3-component-i18n` covers `i18n-loading` + `i18n-formatters`)
...
| §3.4 Internal Dependencies | Package dependency graph | **Read actual `package.json`** deps or run `dependency-cruiser` |
| §3.5 External Dependencies | `package.json` dependencies | **Read actual root + package `package.json`** |
| §3.6 Interactions & Sequences | Spec scenarios with multi-step flows | Verify sequence exists in code (function call chains) |
| §3.7 Database schemas | N/A (client-side library) | Confirm no database code exists |
...
Scan all `openspec/changes/archive/*/design.md` files for `### Decision` headings:
...
1. For each decision:
   ...
   d. Generate ID: `cpt-hai3-adr-{slug}`
...
1. Create file at `architecture/ADR/{NNNN}-{slug}.md`
...
After creating ADRs, update DESIGN §1.2 Architecture Drivers to reference all `cpt-hai3-adr-*` IDs.
...
- [ ] ID unique and follows `cpt-hai3-adr-{slug}` format
...
5. Assign `cpt-hai3-feature-{slug}` IDs

**Coverage enforcement (CRITICAL)**:
- Every `cpt-hai3-component-*` from DESIGN MUST appear in at least one feature
- Every `cpt-hai3-principle-*` from DESIGN MUST appear in at least one feature
- Every `cpt-hai3-constraint-*` from DESIGN MUST appear in at least one feature
...
```markdown
### 2.{N} [{Feature Title}](feature-{slug}/) ⏳ {PRIORITY}

- [ ] `p{N}` - **ID**: `cpt-hai3-feature-{slug}`
...
- [ ] `cpt-hai3-fr-{...}` — {requirement name}

**Design Principles Covered**:
- [ ] `cpt-hai3-principle-{...}` — {principle name}

**Design Constraints Covered**:
- [ ] `cpt-hai3-constraint-{...}` — {constraint name}

**Domain Model Entities**:
- {entity names from DESIGN domain model}

**Design Components**:
- [ ] `cpt-hai3-component-{...}` — {component name}

**API**:
- {public API endpoints or CLI commands}

**Sequences**:
- [ ] `cpt-hai3-seq-{...}` — {sequence name}
...
|---|---|---|
| §2 Actor Flows | `flow` | `cpt-hai3-flow-{feature}-{slug}` |
| §3 Processes | `algo` | `cpt-hai3-algo-{feature}-{slug}` |
| §4 States | `state` | `cpt-hai3-state-{feature}-{slug}` |
| §5 DoD entries | `dod` | `cpt-hai3-dod-{feature}-{slug}` |
...
For each feature, create `architecture/features/feature-{slug}/FEATURE.md`:

1. H1: `Feature: {Feature Name}`
2. `featstatus` ID line directly under H1:
   ```
   - [ ] `p1` - **ID**: `cpt-hai3-featstatus-{slug}`
   ```
...
- Use the language's comment syntax: `// @cpt-flow:cpt-hai3-flow-routing-navigate:p1` for TypeScript
...
Update `.cypilot/config/artifacts.toml` with all artifacts and **FULL traceability**:
...
# Each FEATURE
[[systems.artifacts]]
path = "architecture/features/feature-{slug}/FEATURE.md"
kind = "FEATURE"
traceability = "FULL"
```

Add codebase entries for marker scanning:
```toml
[[systems.codebase]]
path = "packages/"
...
- Invalid ID format → fix to `cpt-hai3-{kind}-{slug}`
...
```bash
python3 .cypilot/.core/skills/cypilot/scripts/cypilot.py spec-coverage
```
...
```bash
python3 .cypilot/.core/skills/cypilot/scripts/cypilot.py toc architecture/PRD.md architecture/DESIGN.md architecture/DECOMPOSITION.md
```
