...
# @cyberfabric/i18n Guidelines (Canonical)
...
- **Default locale policy**: generate `en.json` only. Add extra locales only on explicit user request. `en` is the mandatory fallback.
...
- [ ] Translation file `en.json` exists (mandatory fallback). Extra locales added only on explicit request.
