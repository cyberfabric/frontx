...
- Importing `queryOptions` from `@tanstack/react-query` or `@hai3/react` in MFE code.
- Creating `data/` folders with manual query key factories in MFE packages.
...
- [ ] No `data/` folder with manual query key factories.
