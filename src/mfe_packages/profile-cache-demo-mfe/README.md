...
Copy the entire `profile-cache-demo-mfe` directory to a new name:

```bash
cp -r src/mfe_packages/profile-cache-demo-mfe src/mfe_packages/your-mfe-name
```

### 2. Update Package Metadata

Edit `package.json`:
- Change `"name"` from `"@hai3/profile-cache-demo-mfe"` to `"@hai3/your-mfe-name"`
- Change the port in the `"dev"` and `"preview"` scripts (e.g., from `3002` to your chosen port)

Edit `vite.config.ts`:
- Change `name` in the federation config from `"profileCacheDemoMfe"` to `"yourMfeName"` (camelCase)
...
cp -r src/mfe_packages/profile-cache-demo-mfe /tmp/blank-mfe-validation
