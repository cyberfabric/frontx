 ...
 *
 * The server-state engine is used internally only and is NOT re-exported.
 * MFEs interact with the cache through QueryCache, not the raw runtime.
 */
...
import { useServerStateMutation } from '../serverState';
 ...
 * raw server-state runtime. TContext is the return type of onMutate, passed as
...
>(
  ...
  return useServerStateMutation(options);
}
