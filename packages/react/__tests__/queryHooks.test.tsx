/**
 * Integration tests for TanStack Query–backed API hooks and query-cache integration in @cyberfabric/react - Phase 3
 *
 * Covers:
...
import { useOptionalHAI3QueryClient } from '../src/queryClient';
import {
  createQueryCache,
  type MutationCallbackContext,
} from '../src/hooks/QueryCache';
...
type HiddenQueryClientApp = import('@cyberfabric/framework').HAI3App & {
...
function attachQueryClient(
  app: import('@cyberfabric/framework').HAI3App,
  client: QueryClient
): import('@cyberfabric/framework').HAI3App {
  (app as HiddenQueryClientApp)[APP_QUERY_CLIENT_SYMBOL] = client;
  return app;
}

function getAttachedQueryClient(app: import('@cyberfabric/framework').HAI3App): QueryClient {
  const queryClient = (app as HiddenQueryClientApp)[APP_QUERY_CLIENT_SYMBOL];
  if (!queryClient) {
...
function buildAppWithQueryClient(client: QueryClient): import('@cyberfabric/framework').HAI3App {
  const app = attachQueryClient(createHAI3App(), client);
  ownedApps.push(app);
  return app;
}

function buildPresetApp(): import('@cyberfabric/framework').HAI3App {
...
function buildHostAppWithQueryCache(
  staleTime: number
): import('@cyberfabric/framework').HAI3App {
...
function buildChildAppWithQueryCacheShared(): import('@cyberfabric/framework').HAI3App {
...
describe('HAI3Provider provides query cache access to descendants', () => {
  ...
  it('provider-owned apps still expose mock actions when a QueryClient is attached', () => {
